...
def main():
    ...
    try:
        ...
        s1.push_update_add_entry_to_action(
            ...
            "egress.process_int_transit.init_metadata", [("switch_id", b'\x00\x00\x00\x0c')], 0)
        s1.write_request(req)

        print "Insert Table2 Flow Entry: Add INT report hdr"
        req = s1.get_new_write_request()
        s1.push_update_add_entry_to_action(
            req,
            "egress.process_int_report.tb_generate_report",
            [s1.Exact("int_is_valid", '\x01')],
            "egress.process_int_report.do_report_encapsulation", [("mon_mac", b'\xff\xff\xff\xff\xff\xff'), ("src_ip", b'\x0a\xc0\x13\xb4'), ("src_mac", b'\xa4\x23\x05\x00\x00\x00'), ("mon_ip", b'\x7f\x00\x00\x01'), ("mon_port", b'\xd4\x31')], 0)
        s1.write_request(req)

        print "Insert Table2 Flow Entry: set INT sink"
        req = s1.get_new_write_request()
        s1.push_update_add_entry_to_action(
            req,
            "ingress.process_int_source_sink.tb_set_sink",
            [s1.Exact("standard_metadata.egress_spec", '\x00\x03')],
            "ingress.process_int_source_sink.int_set_sink", [], 0)
        s1.write_request(req)
