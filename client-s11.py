...
def main():
    ...
    try:
        ...
        if not args.skip_role_config:
            ...
            s1.add_roleconfig_entry(roleconfig, "ingress.table2_control.table2", 1)
            s1.handshake(roleconfig)

        # Set Permission ACL
        # print "Insert Ingress Permission ACL entry - Ingress Port == 3 role_id == 1"
        ...
        #     [s1.Ternary("standard_metadata.ingress_port", '\x00\x03', '\x01\xff')],
        #     "permission_acl_ingress.set_user_pipeline_id_and_role_id", [("p_id", b'\x02'), ("r_id", b'\x01')], 100)
        ...
        #     [s1.Ternary("local_metadata.role_id", '\x01', '\x7f'), s1.Ternary("standard_metadata.egress_port", '\x00\x02', '\x01\xff')],
        #     "NoAction", [], 100)
        # s1.write_request(req)
        # 
        # print "Insert Egress Permission ACL entry - Permit role 1 to Egress Port == 3"
        # req = s1.get_new_write_request()
        # s1.push_update_add_entry_to_action(
        #     req,
        #     "egress.permission_acl_egress.permission_acl_egress_table",
        #     [s1.Ternary("local_metadata.role_id", '\x01', '\x7f'), s1.Ternary("standard_metadata.egress_port", '\x00\x03', '\x01\xff')],
        #     "NoAction", [], 100)
        # s1.write_request(req)
        ...
        # print "Insert Egress Permission ACL entry - Drop the other pkts to Egress Port == 3"
        # req = s1.get_new_write_request()
        # s1.push_update_add_entry_to_action(
        #     req,
        #     "egress.permission_acl_egress.permission_acl_egress_table",
        #     [s1.Ternary("standard_metadata.egress_port", '\x00\x03', '\x01\xff')],
        #     "_drop", [], 90)
        # s1.write_request(req)

        # # Set Table1 Flow entry

        # print "Insert Table1 Flow Entry: Port3 => Port2"
        # req = s1.get_new_write_request()
        # s1.push_update_add_entry_to_action(
        #     req,
        #     "ingress.table2_control.table2",
        #     [s1.Ternary("standard_metadata.ingress_port", '\x00\x03', '\x01\xff')],
        #     "ingress.table2_control.set_egress_port", [("port", b'\x00\x02')], 100)
        # s1.write_request(req)

        # print "Insert Table1 Flow Entry: Port2 => Port3"
        # req = s1.get_new_write_request()
        # s1.push_update_add_entry_to_action(
        #     req,
        #     "ingress.table2_control.table2",
        #     [s1.Ternary("standard_metadata.ingress_port", '\x00\x02', '\x01\xff')],
        #     "ingress.table2_control.set_egress_port", [("port", b'\x00\x03')], 100)
        # s1.write_request(req)

        # print "Insert Table2 Flow Entry: Add INT data"
        # req = s1.get_new_write_request()
        # s1.push_update_add_entry_to_action(
        #     req,
        #     "egress.process_int_transit.tb_int_insert",
        #     [s1.Exact("int_is_valid", '\x01')],
        #     "egress.process_int_transit.init_metadata", [("switch_id", b'\x00\x00\x00\x0b')], 0)
        # s1.write_request(req)

        # print "Insert Table2 Flow Entry: set INT source"
        # req = s1.get_new_write_request()
        # s1.push_update_add_entry_to_action(
        #     req,
        #     "ingress.process_int_source_sink.tb_set_source",
        #     [s1.Exact("standard_metadata.ingress_port", '\x00\x03')],
        #     "ingress.process_int_source_sink.int_set_source", [], 0)
        # s1.write_request(req)

        # print "Insert Table2 Flow Entry: init INT source"
        # req = s1.get_new_write_request()
        # s1.push_update_add_entry_to_action(
        #     req,
        #     "ingress.process_int_source.tb_int_source",
        #     [s1.Ternary("hdr.ipv4.src_addr", '\x0a\x00\x00\x01', '\xff\xff\xff\xff'), s1.Ternary("hdr.ipv4.dst_addr", '\x0a\x00\x00\x02', '\xff\xff\xff\xff'), s1.Ternary("local_metadata.l4_dst_port", '\x13\x89', '\xff\xff')],
        #     "ingress.process_int_source.int_source_dscp", [("ins_mask0407", b'\x04'), ("ins_mask0003", b'\x0a'), ("hop_metadata_len", b'\x03'), ("remaining_hop_cnt", b'\x40')], 1000)
        # s1.write_request(req)
